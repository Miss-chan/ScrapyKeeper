...
def generate(args: dict):
    ...
    for root, dirs, files in walk:
        for file in files:
            ...
            abs_dest_dir = abs_dest_dir.replace("master", project_name + "_master"
                                                ).replace("slave", project_name + "_slave")
            mkdir(abs_dest_dir)
            ...
            content = content.replace("{{redis_host}}", args.get('redis_host'))
            content = content.replace("{{redis_port}}", args.get('redis_port'))
